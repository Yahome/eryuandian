...
两元店 Dev OS 的 Markdown 事实源已经建立，T-000 到 T-011 均已完成。TWA-000 Web App MVP 架构冻结与开发基线已完成。TWA-001 已完成技术栈冻结、用户最终确认和 frontend 放行，并已新增国际化 / 双市场版本基线确认；当前仍处于 frontend 第二阶段实现前的架构补充收口。

TWA-001 已完成技术冻结、用户最终确认与国际化 / 双市场版本基线确认；后续重新派发 `yuan-frontend` 时，必须按 locale-aware / market-aware 方向实现移动端优先 App Shell / 首页 / 工作台静态 mock。仍禁止真实登录、短信、生图、试卷生成、支付、额度扣费、真实 API handler、数据库 schema / migration 和 secret。
...
- TWA-001 已完成技术冻结与用户最终确认，owner 已切换为 `yuan-frontend`，状态为“进行中”（内部枚举：`in_progress`），progress 为 `60`。
- `yuan-frontend` 已正式放行；后续重新派发 frontend 时必须按 locale-aware / market-aware 方向实现。
