...
TWA-001 当前重点是技术栈、数据库、部署基准、100 用户同时在线基准、移动端优先设计基线和 App Shell / Mock / Adapter 边界。`yuan-frontend` 暂不允许开工；本轮不实现 UI，不新增真实 API 文件，不修改数据库 schema，不接真实短信 / 生图模型 / 试卷模型 / 支付。
...
- 当前状态：“进行中”（内部枚举：`in_progress`），owner 为 `yuan-architect`，progress 为 `40`。
- 当前产物：`docs/project-os/tasks/web-app/TWA-001-Web-App-技术栈冻结与移动优先-App-Shell-基础.md`。
- 当前重点：技术栈、数据库、部署基准、100 用户同时在线基准、移动端优先设计基线、App Shell 范围边界、Mock / Adapter 组织方式。
- architect 前置结论：建议采用 React + Vite + TypeScript + React Router + TanStack Query，后端 Node.js 单体 + Hono/Fastify，数据库优先 PostgreSQL，ORM 优先 Drizzle；移动端必须从 TWA-001 起作为主设计目标，frontend 暂不允许开工。
- `yuan-frontend` 暂不允许开工；等待用户确认和 reviewer 无 blocker。
...
- 当前阶段：Phase 1 / TWA-001 技术栈冻结与移动优先基线进行中
...
- TWA-001 已创建并派发 `yuan-architect` 前置架构说明。
- TWA-001 当前不进入 UI 实现；`yuan-frontend` 需等待技术栈、数据库、移动端优先原则、App Shell 范围和后续任务顺序经用户确认后再开工。
