...
本节是 TWA-002B / TWA-002C 的唯一实现依据。文档后续若出现“草案”“历史规划”或 TWA-000 旧 Auth contract，均只用于追溯；若与本节冲突，以本节为准。

### 路由与页面结构

- 登录页：`/[locale]/login`，例如 `/zh-CN/login`、`/en/login`。
- 登录 / 注册同页 Tab，不创建 `/[locale]/register`。
- 保留 `return_to?: string | null`。
- 登录成功后优先跳转 `return_to`，否则默认跳转 `/{locale}/app/dashboard`。
- 未登录访问 `/{locale}/app` 与 `/{locale}/app/dashboard` 时，当前阶段保持静态可见并展示 mock session 状态，不做真实 redirect。
...
旧接口 `/api/auth/login`、`/api/auth/sms-code` 仅保留历史追溯，不作为实现基准。
...
- `GET /api/auth/session` 有有效会话：`success: true`，`data: { session: SessionDTO, user: UserDTO }`。
- `GET /api/auth/session` 无会话：HTTP 200，`success: true`，`data: { session: null, user: null }`。
- 携带过期会话：HTTP 401，error code 为 `SESSION_EXPIRED`。
- 当前冻结契约不使用 `authenticated: true` 或 `authenticated: false` 字段。
- `POST /api/auth/logout` 无 body，由当前 session context 注销。
- `LogoutResponse` 统一为 `{ logged_out: true, cleared: true }`，不得只返回 `{ cleared: true }`。
...
- TWA-002B / TWA-002C 可并行，但必须以本节与 `API_CONTRACT.md` 为唯一契约依据。
- 不得各自发明 DTO。
- 不得引入 `captcha_token`、`authenticated` 或旧 `/api/auth/login` 作为当前实现基准。
- 当前仍禁止真实 API handler、DB schema / migration、短信 provider、真实登录生产接入和 secret。

## Mock / Adapter 边界（历史规划草案）
...
以下为历史规划草案，当前实现以“当前实现前冻结结论（唯一执行基准）”为准；本段不代表真实 API handler 已实现：
...
- 冻结 endpoints（仅此四个）：
  ...
  - `GET /api/auth/session`
  - `POST /api/auth/logout`
- 历史保留但禁用：旧 `POST /api/auth/login` 与 camelCase（`codeId` / `expiresAt`）仅作历史参考，不进入实现。
...
本节仅补充说明 Public DTO 与 `DATA_MODEL.md` 内部概念模型的边界；具体字段以文档前部“当前实现前冻结结论（唯一执行基准）”与 `API_CONTRACT.md` 为准，避免重复定义造成漂移。
...
其中 `LoginPhoneRequest` 必须包含 `return_to?: string | null`，`LoginPhoneResponse` 必须包含 `redirect_to: string`，`SessionResponse` 必须为 `{ session: SessionDTO | null; user: UserDTO | null }`。

`Public DTO` 是对外 API 稳定契约；`DATA_MODEL.md` 是内部概念模型（如验证码哈希、尝试次数、provider 原始回包、风控标签、会话存储键），不可直接外露。
